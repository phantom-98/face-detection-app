...
int get_aligned_face(const cv::Mat& img, float* pLandmarksX, float* pLandmarksY, int landmark_number, int desired_size, cv::Mat& out)
{
   ...
   if(cal_scale_and_angle(pLandmarksX, pLandmarksY, landmark_number, desired_size, &scale, &angle) < 0)
        return -1;

   to_center[0] = desired_size * 0.5;
   to_center[1] = desired_size * 0.4;

   from_center[0] = (pLandmarksX[0] + pLandmarksX[1]) / 2;// (landmark[0] + landmark[1]) / 2;
   from_center[1] = (pLandmarksY[1] + pLandmarksY[2]) / 2;// (landmark[5] + landmark[6]) / 2;
