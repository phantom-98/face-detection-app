...
open class GraphicOverlay(context: Context?, attrs: AttributeSet?) :
    ...
    abstract class Graphic(private val overlay: GraphicOverlay) {
        ...
        fun calculatePoint(height: Float, width: Float, point: PointF): PointF{
            ...
            var pointF = PointF(point.x*scale + offsetX, point.y*scale + offsetY)
            if (overlay.isFrontMode()) pointF.x = overlay.width - pointF.x
    ...
    fun toggleSelector(mode: CameraMode){
        cameraSelector =
            if (mode == CameraMode.KIOSK) CameraSelector.LENS_FACING_FRONT
            else CameraSelector.LENS_FACING_BACK
    }
