...
int init_landmark_vanila(AAssetManager *assetManager) {
    g_face_model_extractor = new FaceModelExtractor(assetManager, 60, 60, 4);
...
void get_face_68Landmark2(unsigned char* pbFace, int iExFaceW, int iExFaceH, float* landmark_ptr)
{
	...
	g_face_model_extractor->extract(rData, feat2);
	for (int i = 0; i < 68; i++) {
		landmark_ptr[2 * i] = feat2[2 * i] * iExFaceW;				// xPos
		landmark_ptr[2 * i + 1] = feat2[2 * i + 1] * iExFaceH;		// yPos
	}
	free(feat2);
