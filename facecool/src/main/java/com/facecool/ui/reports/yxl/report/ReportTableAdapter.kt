...
class ReportTableAdapter constructor(
    ...
    private fun getAttendanceView(parent: ViewGroup, attendance: String): TextView {
        ...
        when (attendance) {
            parent.context.getString(R.string.class_report_present_label),
            parent.context.getString(R.string.class_report_Late_label) -> binding.itemData.setBackgroundColor(Color.GREEN)
            else -> binding.itemData.setBackgroundColor(Color.WHITE)
        }
        return binding.root
    }
    ...
    override fun getItemViewType(row: Int, column: Int): Int {
        if (row == 0 && (column == 0 || column==1)) return TYPE_EMPTY
        if (row == 0) return TYPE_DATE
        if (column == 0) return TYPE_PHOTO
        if (column == 1) return TYPE_NAME
