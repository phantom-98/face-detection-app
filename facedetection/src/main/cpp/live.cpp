...
void Live::SetCheckLiveness(bool isCheckLiveness) {
    m_isCheckLiveness = isCheckLiveness;
}

float Live::Detect(cv::Mat &src, cv::Rect rtFace) {
    float confidence = 0.f;
    if( model_num_ == 0 || m_isCheckLiveness == false)
        return confidence;
    for (int i = 0; i < model_num_; i++) {
        cv::Mat roi;
        if(configs_[i].org_resize) {
            ...
            cv::Rect rect = CalculateBox(rtFace, src.cols, src.rows, configs_[i]);
            // roi resize
...
cv::Rect Live::CalculateBox(cv::Rect rtFace, int w, int h, ModelConfig &config) {
    int x = rtFace.x;// static_cast<int>(box.x1);
    int y = rtFace.y;//static_cast<int>(box.y1);
    int box_width = static_cast<int>(rtFace.width);//(box.x2 - box.x1 + 1);
    int box_height = static_cast<int>(rtFace.height);//(box.y2 - box.y1 + 1);
