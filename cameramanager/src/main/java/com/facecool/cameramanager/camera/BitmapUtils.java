...
public class BitmapUtils {
  ...
  public static Bitmap getBitmap(ByteBuffer data, FrameMetadata metadata) {
    ...
    Constants.LogDebug(String.format("Frame (Width x Height) : (%d, %d)", metadata.getWidth(), metadata.getHeight()));
    Constants.LogDebug(String.format("Frame Rotation : %d", metadata.getRotation()));
    ...
    try {
      ...
      Constants.LogDebug("getBitmap() Error: " + e.getMessage());
    }
    return null;
  }
...
//  }
  ...
  private static int getExifOrientationTag(ContentResolver resolver, Uri imageUri) {
    ...
    try (InputStream inputStream = resolver.openInputStream(imageUri)) {
      ...
      Constants.LogDebug("getExifOrientationTag() failed to open file to read rotation meta data: " + imageUri + "\nError : " + e.getMessage());
      return 0;
    }
