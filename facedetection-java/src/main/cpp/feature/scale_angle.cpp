...
static int get_probe_vec(float *pLandmarkX, float * pLandmarkY, int landmark_number, float * probe_vec, int probe_size, int desired_size) {
    int i;

   float mean_face_x[] = { 38.2946, 73.5318, 56.0252, 41.5493, 70.7299};
   float mean_face_y[] = { 51.6963, 51.5014, 71.7366, 92.3655, 92.2041 };
...
//    }
    for (i = 0; i < 5; i++) {
        from_mean[0] += pLandmarkX[i];
        from_mean[1] += pLandmarkY[i];//[i+5];
    ...
    for (i = 0; i < 5; i++) {
        ...
        gap_from[0] = pLandmarkX[i] - from_mean[0];
        gap_from[1] = pLandmarkY[i] - from_mean[1];
...
int cal_scale_and_angle(float *pLandmarkX, float *pLandmarkY, int landmark_number, int desired_size,float * scale, float * angle)
{
   float probe_vec[2];

   if(get_probe_vec(pLandmarkX, pLandmarkY, landmark_number, probe_vec,2, desired_size)<0)
      return -1;

   scale[0] = frob_norm(probe_vec,2);
   angle[0] = 180.0 / 3.14 * atan2(probe_vec[1],probe_vec[0]);
